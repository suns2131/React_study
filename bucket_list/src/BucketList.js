...
const my_wrap = React.useRef(3); //REF선언 



window.setTimeout(() => {
    console.log(my_wrap);
},1000); //두번째 인자만큼의 시간이 흐른 후에 첫번째 인자로 넘겨준 함수를 실행하라는 윈도우 내장함수 .. 밀리세컨드 기준
...
my_lists.map((list_a, index) => {
// 콘솔을 확인해봅시다 :)
console.log(list_a);
return (
<div  className='wish_list' key={index}>{list_a}</div>);
})
}
</div>
);
